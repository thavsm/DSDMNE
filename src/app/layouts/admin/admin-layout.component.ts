...
export class AdminLayoutComponent implements OnInit, AfterViewInit {
    ...
    ngOnInit() {
        ...
        /* this._router = this.router.events.filter(event => event instanceof NavigationEnd).subscribe((event: any) => {
             elemMainPanel.scrollTop = 0;
             elemSidebar.scrollTop = 0;
        });         */
        this._router = this.router.events.filter(event => event instanceof NavigationEnd).subscribe({
         
          next:   () => {  elemMainPanel.scrollTop = 0;
            elemSidebar.scrollTop = 0;},
